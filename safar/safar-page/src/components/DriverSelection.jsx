...
export default function DriverSelection({
  ...
  const fetchDisponibilidad = async () => {
    ...
    try {
      ...
      try {
        ...
        // PHP devolvió algo que no es JSON — mostrar primeros 300 chars para diagnóstico
  ...
  return (
    <div>
      ...
      <div style={{ background: '#f8f8f8', borderRadius: 16, padding: 16, marginBottom: 16 }}>
        ...
        {recomendacion && (
          <div style={{
            ...
            <Star size={14} color="#f59e0b" style={{ flexShrink: 0 }} />
            ...
            >
              {isRecomendado && (
                <div style={{
                  ...
                  padding: '3px 10px', borderRadius: '0 14px 0 12px',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                <div style={{
                  ...
                  color: isSelected ? '#111' : '#fff', flexShrink: 0,
                }}>
                  <User size={20} />
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 2 }}>{chofer.nombre}</div>
              ...
              <div style={{ marginTop: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 4 }}>
                  <span style={{ opacity: 0.7 }}>{chofer.viajes_en_ventana} / {chofer.max_viajes} viaje/hr</span>
                  <span style={{ fontWeight: 700, color: colorCarga }}>{chofer.porcentaje_carga}%</span>
                </div>
                <div style={{ height: 6, borderRadius: 3, background: isSelected ? 'rgba(255,255,255,0.2)' : '#f0f0f0', overflow: 'hidden' }}>
                  <div style={{ width: `${chofer.porcentaje_carga}%`, height: '100%', background: colorCarga, borderRadius: 3, transition: 'width 0.3s ease' }} />
                </div>
              </div>

              {chofer.disponible && (
                <div style={{ marginTop: 8, fontSize: 11, color: isSelected ? 'rgba(255,255,255,0.8)' : '#22c55e', fontWeight: 600 }}>
                  ✓ {chofer.slots_disponibles} slot{chofer.slots_disponibles !== 1 ? 's' : ''} disponible{chofer.slots_disponibles === 1 && ' — ¡Reserva pronto!'}
                </div>
              )}
              {!chofer.disponible && (
      ...
      <div style={{ marginTop: 16, fontSize: 11, color: '#888', textAlign: 'center', padding: '8px 0' }}>
        {disponibles === total
