...
export function SingleServiceLayout({
  id, tag, name, tagline, description, image,
  features, price, phone, facebook, instagram, cta, accent, theme,
  externalLink, imgScale = 1, imgFilter = 'none'
}) {
  useScrollReveal();
  ...
  return (
    <>
      <WhatsAppModal 
        ...
        projectName={name}
        planInfo={{ label: 'Servicio', value: name }}
        price={price}
        accent={accent}
        textMuted={textMuted}
        waNumber={waNumber}
          ...
          <div style={{ padding: '3rem 3.5rem' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', justifyContent: 'space-between', gap: '1rem', marginBottom: '1.2rem' }}>
              <div>
                ...
                {tagline && <p style={{ fontSize: '1.2rem', color: accent, fontWeight: '700', margin: '0.6rem 0 0' }}>{tagline}</p>}
              </div>
              {phone && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', color: textMuted, fontSize: '0.9rem', background: 'rgba(0,0,0,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '99px', padding: '0.5rem 1.2rem' }}>
                    <span>📞</span>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2rem', alignItems: 'center', justifyContent: 'center' }}>
              {price && (
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '0.7rem', color: textMuted, fontWeight: '700', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.3rem' }}>Mensualidad</div>
                  <div style={{ fontSize: '2.8rem', fontWeight: '900', color: accent, lineHeight: '1' }}>{price}</div>
                </div>
              )}
              <button
                onClick={() => externalLink ? window.open(externalLink, '_blank') : setIsModalOpen(true)}
                style={{ background: accent, color: theme === 'yellow' || theme === 'gold' ? 'black' : 'white', padding: '1.2rem 3rem', borderRadius: '99px', fontSize: '1.1rem', fontWeight: '800', border: 'none', cursor: 'pointer', transition: 'all 0.3s', flexGrow: 1, maxWidth: '400px' }}
                onMouseEnter={e => (e.currentTarget.style.filter = 'brightness(1.15)')}
                onMouseLeave={e => (e.currentTarget.style.filter = 'none')}
              >
                {cta}
